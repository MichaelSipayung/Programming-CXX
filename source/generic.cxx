...
void findAl(const std::vector<int>&x){
    if (!x.empty())
    {
        ...
        std::cout<<"The value "<<val << (result==x.cend()
            ? " is not present" : "is present")<<std::endl;
        std::list<std::string> author_name  = {"Miller","Jack","Potret","Goodfellow","Anton"};
        std::string find_auth = "miller";
        auto result_auth  = std::find(author_name.begin(), author_name.end(),find_auth);
        ...
        {
            std::cout<<"oops nothing to search, this is empty container"<<std::endl;
        }
        int find_nat= 8;
        auto result_nat  = std::find(std::begin(nat_num), std::end(nat_num),find_nat);
    ...
    if (result_isbn==isbn.cend())
    {
        ...
        std::cout<<"success, found char : " << c << " in  : " << isbn<<std::endl;
    }
}
...
void elimDups(std::vector<std::string>&x){
    ...
    //size is unchanged
    auto end_unique = std::unique(x.begin(),x.end()); //point to the last non unique element
...
void lamdaWithArgs(){
    ...
    std::vector<std::string> auth_name  ={"jack","miller","doss","fos","joe","joel","doss"};
    ...
    std::stable_sort(auth_name.begin(),auth_name.end(),
        [](const std::string&a,const std::string &y){return a.size() <y.size();});
...
void useCaptureList(const std::vector<std::string> &x,std::string::size_type sz){
    //[capture list]... only variable in the capture list can appear in the func body
    ...
    std::for_each(wc,x.end(),[](const std::string &s){std::cout<<s<<" ";});
}
void capByVal(){
...
void capByRef(std::vector<std::string> &word, std::string::size_type s,
    ...
    std::for_each(word.begin(), word.end(),[&os,c](const std::string &s)
    {
        os<<s<<c;
    });
}
//implicit reference
void implicitCapture(std::vector<std::string>&word,std::vector<std::string>::size_type sz,
    std::ostream&os,char c=' '){
    //os implicitly captured by reference; c explicitly captured by value
    std::for_each(word.begin(), word.end(),[&,c](const std::string &s){os<<s<<c;});
}
//mutable lambda
