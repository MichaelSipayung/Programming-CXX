...
template<typename Object>
void iter_ostream(std::ostream_iterator<Object> &out_iter, const std::vector<Object> &value)
{
    for(auto v:value)
...
template<typename Container>
void iter_reverse(const Container& data)
{
    ...
    while(begin!=end)
        std::cout<<*begin++<<" "; //using postfix operator, need not to change the state
}
...
void iter_relation(const std::string&data)
{
    ...
    auto foundIt = std::string(rcomma.base(),data.cend()); //base [data ....base find... end]
