...
// generate synthesized version of the copy control members by defining them as
// =default
...
// prevent copies by defining the copy constructor and copy assignment as del
// functions
class no_copy {
public:
  no_copy() = default;
...
// demonstrate class that act like values, this version is modification of
// hash_ptr
class hash_ptr_value {
public:
  explicit hash_ptr_value(const string &s = string())
      : ps_(new string(s)), i_(0) {}
...
public:
  ...
  explicit hash_ptr_pointer(const string &s = string())
