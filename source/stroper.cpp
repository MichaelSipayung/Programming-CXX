...
void insertStr(std::string &x) {
  ...
  // using replace
  x.replace(11, 3,
            "5th"); // equal to calling erase, replace str from pos 11 with 5th
}
// search operation
void findStr(const std::string &x) {
  ...
  auto posFind =
      name.find_first_of(numbers); // return 1 the index of first digit in name
...
void numConver() {
  int i = 42;
  std::string s =
      std::to_string(i); // convert the integer to character represent
  ...
  std::cout << "find in the complicated expression (convert) : " << numCon * 2
            << std::endl;
}
