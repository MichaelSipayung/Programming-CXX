...
//another ilustration 
bool check_another(const std::string &word, int x)
{
    return (word.size() < x);
}
...
void test_other_f()
{
    auto wc = std::bind(other_f,2,3,std::placeholders::_2,4,std::placeholders::_1);
    wc(3,5); //call by reverse order for std::bind
}
bool other_f1(int x, int y, const std::string &word, int z, const std::string &otherW)
{
    if (word.size() < (x+y))
        return false;
    else
...
void test_other_f1()
{
    auto wc  = std::bind(other_f1,2,3,std::placeholders::_2,5,std::placeholders::_1);
    //call wc(x,y)  == f(2,3,y,5,x) //reverse order
    std::cout<<"test how the object arrange in std::bind : " << wc("jackie","geo")
        <<std::endl;
}
...
{
    std::for_each(word.begin(), word.end(),std::bind(bind_print,std::ref(os),
        std::placeholders::_1,' ')); //using std::ref(reference obj) for bind reference arg
}
bool bind_corrected(int x, int y, const std::string &word, int x_1, const std::string word_1)
{
    if((x+y) < word.size())
...
void test_bind_corr()
{
    auto w_test = std::bind(bind_corrected,2,3,std::placeholders::_2, 4,std::placeholders::_1);
    std::cout<<"test 1: "<< w_test("back arg","front args")<<std::endl; //reverse order to assign & call
}
#endif