...
void init_con() {
  ...
  std::vector<std::string> cast_articl(articl.begin(),
                                       articl.end()); // initialize
}
void init_con_arr() {
...
void check_size() {
  ...
  if (demo_size_op.empty()) {
    std::cout << "using emepty to check if container has no element"
              << std::endl;
  }
  if (demo_size_op.max_size() < 1e3) {
    std::cout << "using max_size to check total element \
            can hold in container"
              << std::endl;
  }
}
void relat_op() {
...
void add_el() {
  ...
  std::cout << "after add in the end of the string (size) : " << book_num.size()
            << std::endl;
  ...
  nega_num.insert(nega_num.end(), 10,
                  -10); // insert -10 ten times to the end of the element
...
void acc_el() {
  ...
  if (!name.empty()) {
    ...
    std::cout << "using front to access the first element : " << name.front()
              << std::endl;
    std::cout << "access the last element using back : " << name.back()
              << std::endl;
  }
...
void err_el() {
  ...
  if (!item_num.empty()) {
    std::cout << "errase the item_num element: " << item_num.back()
              << std::endl;
    // errase the element
    item_num.pop_back();
  ...
  if (!nat_num.empty()) {
    std::cout << "erase the element in the first position : " << nat_num.front()
              << std::endl;
    nat_num.pop_front();
  }
  // using erase
  ...
  if (!author_name.empty()) {
    std::cout << "using erase to remove the element : " << *author_name.begin()
              << std::endl;
  ...
  std::cout << "string case, erase the first element : " << *nick_nm.begin()
            << std::endl;
...
void rem_by_pair() {
  ...
  num.erase(num.begin(),
            ++num.begin()); // remove the first element and the next element
}
void forw_list_it() {
  ...
  if (!nat_num.empty()) {
    for (auto first_el = nat_num.begin(); first_el != nat_num.end();
         ++first_el) {
      std::cout << " | " << *first_el << " ";
    }
  }
...
void manage_cap() {
  ...
  if (!nat_num.empty()) {
    ...
    nat_num.reserve(50);
    std::cout << "after reserve (capacity): " << nat_num.capacity()
              << std::endl;
  }
}
