...
void refactor::initialize_direct() {
  const auto pi = new int;    // pi point to a dynamically allocated, unnamed,
                              // uninitialized int
  const auto ps = new string; // ps points to empty string
...
void refactor::initialize_const_direct() {
  ...
  auto ps =
      new const string; // allocate a default-initialized const empty string
}
...
void refactor::memory_exhaust() {
  auto pi = new int(12); // should throw if allocate fail
  // auto ps = new (nothrow)string("test"); //if allocation fails, new return a
  // null pointer
}
...
// avoid using built in pointer, when the exception occurs the variable will not
// freed
...
void refactor::bad_ptr() {
  const auto x =
      new int(12); // ok, if exception occurs before we freed resource!
...
void refactor::init_unique_ptr() {
  ...
  const std::unique_ptr<string> p3 =
      std::move(p2); // prefer move rather than reset
  std::cout << *p3;
}
// copy and assignment in unique_ptr in object is about to destroyed
std::unique_ptr<int> refactor::clone(int p) {
  return std::make_unique<int>(
      p); // compiler know that the object is about to destroyed
}
