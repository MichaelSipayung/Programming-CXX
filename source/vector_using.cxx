...
void introToVector(void){
    ...
    ivec[9]=9;
    std::cout<<"start element : " <<ivec[0]<<" .... " <<ivec[9]<<std::endl;
}
...
void byIterator(void){
    ...
    std::cout<<"after traverse the element using string : "<< name<<std::endl;
...
void advAray(void){
    ...
    std::cout<<"after safe copy to the destination : " << destination<<std::endl;
...
void interfaceOldC(void){
    ...
    std::cout<<"here we are demonstrate how to interface the c code to c++"<<std::endl;
    for(auto a:ivec){
        std::cout<<a<<" | ";
    }
    std::cout<<std::endl;
