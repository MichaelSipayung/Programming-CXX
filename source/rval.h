...
public:
  ...
  Rhs_1(Rhs_1 &&p) noexcept : xPoint{p.xPoint}, yPoint{p.yPoint} {
    p.xPoint = 0;
    p.yPoint = 0;
  }
...
inline void Rhs_1::push_back(const double &s) {
  xPoint += s;
  yPoint += s;
}
inline void Rhs_1::push_back(double &&s) {
  xPoint = std::move(s);
  yPoint = std::move(s);
}
inline void Rhs_1::demo_push() {
  Rhs_1 testX;
...
inline Rhs_1 &Rhs_1::operator=(Rhs_1 mvCp) {
  using std::swap;
  swap(*this, mvCp);
  return *this;
}
...
inline Ref_Mem_Fun &Ref_Mem_Fun::operator=(const Ref_Mem_Fun &s) & {
  // do assign operation .
  return *this;
}
inline void Ref_Mem_Fun::testLvalRef() {
  double x = 35.9;
...
inline void Foo_B::demo_Overload() {
  Foo_B val;
  Foo_B &refLval = val;
