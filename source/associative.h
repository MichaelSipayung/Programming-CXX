...
void wordCounting_map(std::map<Object, size_t> &word) {
  ...
  while (std::cin >> key)
    ++word[key];
  for (const std::pair<std::string, size_t> &w :
       word) // using std::pait ro read each key value on map
...
template <typename Object>
void elemin_set(std::map<Object, size_t> &word_count,
                const std::set<Object> &excl) {
  // test case: string
  const std::vector<std::string> &test_elim = {
      "and", "the", "bird", "song", "in", "the", "night", "alone", "a"};
...
template <typename Object>
void diff_multiSet(const std::vector<Object> &data,
                   const std::set<Object> &set_obj,
                   const std::multiset<Object> &mulSet_obj) {
  std::cout << " vector size : " << data.size() << std::endl;
  std::cout << " set size (not allowed dupplicate key): " << set_obj.size()
            << std::endl;
...
void moreType_pair(const std::pair<Object_1, Object_2> &ex_pair) {
  std::cout << "first : " << ex_pair.first << " second : " << ex_pair.second
            << std::endl;
}
// return pair object
...
template <typename Object_1, typename Object_2>
void typeAliases_assCon(const std::set<Object_1> &x,
                        const std::map<Object_1, Object_2> &y) {
...
void iteratorMap_assoc(std::map<Object_1, Object_2> &x, const Object_2 &val) {
  ...
  std::cout << "begin iterator (key) : " << map_it->first
            << " begin iterator (value) : " << map_it->second << std::endl;
...
void iterElement_assoc(const std::map<Object_1, Object_2> &x) {
  ...
  while (begin != end) {
    std::cout << "first : " << begin->first << " second : " << begin->second
              << std::endl;
    ++begin;
  }
}
...
template <typename Object_1, typename Object_2>
void addElement_map(const std::vector<Object_1> &source_key,
                    const std::vector<Object_2> &source_val,
...
template <typename Object_1, typename Object_2>
void workW_multimap(std::multimap<Object_1, Object_2> &auth,
                    const Object_1 &x_1, const Object_2 &x_2,
                    const Object_2 &x_3) {
  // case 1
  auth.insert({x_1, x_2});
...
template <typename Object_1, typename Object_2>
void accesEl_assoc(const std::set<Object_1> &x,
                   const std::map<Object_1, Object_2> &y, const Object_1 &z) {
...
template <typename Object_1, typename Object_2>
void find_multiKey(const std::multimap<Object_1, Object_2> &x,
                   const Object_2 &src) {
...
template <typename Object_1, typename Object_2>
void find_multiKey_d(const std::multimap<Object_1, Object_2> &x,
                     const Object_2 &src) {
  auto beg = x.lower_bound(src);
  auto end = x.upper_bound(src);
  while (beg != end) {
    std::cout << beg->second << std::endl; // fetch each value with the given
                                           // key
    ++beg;
  }
}
...
template <typename Object_1, typename Object_2>
void unorCon_print(const std::unordered_map<Object_1, Object_2> &x,
                   const Object_1 &key) {
  ...
  if (find_key == x.end())
    ...
    std::cout << "oh found it ...it's value :  " << find_key->second
              << std::endl;
}
#endif