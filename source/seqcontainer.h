...
void clearEvenNum()
{
  ...
  while (begin != end) {
    if (*begin % 2 == 0)
      begin=num_ls.erase(begin);//erase this element, begin point to the next elem
    else
      ++begin;
  }
...
void singlyLinkedList() {
  forward_list<string> lst = { "element 1","element 2","element 3","element 4" };
...
void dynChange_container()
{
  ...
  while (iter != vi.end()) {
    if (*iter % 2) {
      ...
      iter = vi.erase(iter); //rm even element, return the next iterator afer iter iterator
  }
...
void testQueue() {
  ...
  singer_name.pop();
  cout << "remove using fifo policy (first singer) -> " << singer_name.front() << endl;
}
//priority queue
void prioQueTest() {
