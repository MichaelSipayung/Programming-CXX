...
void switStatement(void) {
  const std::string letter =
      "Allow only one make using nested if, i get grade letterGrade";
  ...
  std::cout << "Other characters: "
            << (letter.size() - (aCnt + eCnt + iCnt + oCnt + uCnt))
            << std::endl;
}
void modifySwitchStatement(void) {
  ...
  std::cout << "modify vowel counting program, (vowel total)  : " << totalVowel
            << std::endl;
...
void rangeForStatement(void) {
  ...
  std::cout << "using range for statement to iterate and change the object"
            << std::endl;
...
void doWhileStatement(void) {
  ...
  do {
    ...
    int x = 0, y = 0;
    std::cin >> x >> y;
    std::cout << "the sum of " << x << " and " << y << " = " << (x + y)
              << std::endl;
